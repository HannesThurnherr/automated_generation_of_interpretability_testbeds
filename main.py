...
from GeminiChatModel import GeminiChatModel
from AnthropicChatModel import AnthropicChatModel
from OpenAIChatModel import OpenAIChatModel
import resource
...
def run_with_timeout(func, args=(), kwargs={}, timeout=None):
    """
    Run function in separate thread and throws error if the execution takes longer than the specefied number of seconds.
    """
    thread = ThreadWithReturnValue(func, args, kwargs)
    thread.start()
    thread.join(timeout)
    if thread.is_alive():
        raise TimeoutError("Error in defining op")
    else:
        return thread.join()
...
def process_task(model: AssembledTransformerModel, task_data: dict, promt_file: str, tries:int=5, verbose:bool=True):
    ...
    for i in range(tries):
        ...
        try:
            ...
            result = run_with_timeout(check_op_runs, args=(op,), kwargs={'verbose': verbose}, timeout=30)
            if result is None:
...
def evaluate_model(model, verbose: bool = True):
    ...
    with open(f'evaluation_results_{model.model_name}_{datetime.now().strftime("%Y%m%d")}.json', 'w') as f:
        for key in  tqdm(list(data.keys())[92:], desc="Evaluating Tasks"):
            ...
            # Update the file with the current results at each iteration
            f.seek(0)  # Move the file pointer to the beginning
            json.dump(results, f, indent=4)
            f.truncate()  # Truncate any remaining content after the written data
...
def visualize_results(results: dict):
    ...
    ratio_text =  'Completed Tasks: '+str( len([key for key in results["successes"].keys() if len(results["successes"][key])>0]))+" / "+str(len(list(results["successes"].keys())))
    with open('eval_data.json', 'r') as file:
        data = json.load(file)
    score = sum([data[key]["number_of_rasp_functions"]*int(len(results['successes'][key]) > 0) for key in results['successes'].keys()])
    diff_scaled_text = f"Score: {score}"
    axs[1, 1].text(0.5, 0.5,  ratio_text+"\n"+diff_scaled_text,
...
openai_model_names = [
    ...
    "gpt-4-1106-preview", "gpt-4-vision-preview", "gpt-4-1106-vision-preview", "gpt-4",
    "gpt-4-0613", "gpt-4-32k", "gpt-4-32k-0613", "gpt-3.5-turbo-0125", "gpt-3.5-turbo",
    "gpt-3.5-turbo-1106", "gpt-3.5-turbo-instruct", "gpt-3.5-turbo-16k", "gpt-3.5-turbo-0613",
    "gpt-3.5-turbo-16k-0613", "gpt-4o-2024-05-13", "gpt-4o"
]
...
if len(sys.argv) > 1:
    ...
    if model_name in openai_model_names:
        ...
        model = AnthropicChatModel(model_name)
    elif model_name in google_model_names:
        model = GeminiChatModel(model_name)
    else:
        model_found = False
        print("Model name not found. Add to list of model names in main.py or call 'evaluate_model(model)' directly with your own implementation of the abstract_model class")
